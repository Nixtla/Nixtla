...
__all__ = ['NBeatsBlock', 'NBeats', 'IdentityBasis', 'TrendBasis', 'SeasonalityBasis', 'ExogenousBasisInterpretable',
           'Chomp1d', 'ExogenousBasisWavenet', 'XBasisTCN', 'XBasisWavenet']
...
class NBeatsBlock(nn.Module):
    ...
    def __init__(self, x_t_n_inputs: int, x_s_n_inputs: int, x_s_n_hidden: int, theta_n_dim: int, basis: nn.Module, # n_static:int
                 n_theta_hidden_list: list, theta_with_exogenous: bool, batch_normalization: bool, dropout_prob: float, activation: str):
        """
        """
        super().__init__()
        ...
        for i in range(len(n_theta_hidden_list)-1):
            hidden_layers.append(nn.Linear(in_features=n_theta_hidden_list[i], out_features=n_theta_hidden_list[i+1]))
        ...
        output_layer = [nn.Linear(in_features=n_theta_hidden_list[-1], out_features=theta_n_dim)]
...
class XBasisTCN(nn.Module):
    def __init__(self, out_features, f_idxs, num_levels=4, kernel_size=2, dropout_prob=0):
        super().__init__()
        self.f_idxs = f_idxs
...
class XBasisWavenet(nn.Module):
    def __init__(self, out_features, f_idxs, num_levels=4, kernel_size=3, dropout_prob=0):
        super().__init__()
        # Shape of (1, in_features, 1) to broadcast over b and t
        self.f_idxs = f_idxs
        in_features = len(self.f_idxs)
