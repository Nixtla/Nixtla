# AUTOGENERATED! DO NOT EDIT! File to edit: nbs/models_wnbeats__nbeats_model.ipynb (unless otherwise specified).

__all__ = ['create_context', 'NBeatsBlock', 'NBeats', 'IdentityBasis', 'TrendBasis', 'SeasonalityBasis',
           'ExogenousBasisInterpretable', 'ExogenousBasisWavenet', 'XBasisTCN', 'XBasisWavenet']
...
from ..component import Chomp1d, TemporalConvNet

# Cell
def create_context(insample_y, insample_x_t, outsample_x_t, t_cols, include_var_dict):
...
class NBeatsBlock(nn.Module):
    ...
    def __init__(self, x_t_n_inputs: int, x_s_n_inputs: int, x_s_n_hidden: int,
                 theta_n_dim: int, basis: nn.Module, n_theta_hidden_list: list,
                 include_var_dict: dict, t_cols: list, batch_normalization: bool,
        ...
        if x_s_n_inputs == 0:
            x_s_n_hidden = 0
        n_theta_hidden_list = [x_t_n_inputs + x_s_n_hidden] + n_theta_hidden_list

        hidden_layers = []
        for i in range(len(n_theta_hidden_list)-1):
            hidden_layers.append(nn.Linear(in_features=n_theta_hidden_list[i],
                                           out_features=n_theta_hidden_list[i+1]))
            hidden_layers.append(self.activations[activation])

            if self.batch_normalization:
                hidden_layers.append(nn.BatchNorm1d(n_theta_hidden_list[i+1]))

            if self.dropout_prob>0:
                hidden_layers.append(nn.Dropout(p=self.dropout_prob))

        output_layer = [nn.Linear(in_features=n_theta_hidden_list[-1],
                                  out_features=theta_n_dim)]
        ...
        if (self.x_s_n_inputs > 0) and (self.x_s_n_hidden > 0):
            self.static_encoder = _StaticFeaturesEncoder(in_features=x_s_n_inputs,
    ...
    def forward(self, insample_y: t.Tensor, insample_x_t: t.Tensor,
        ...
        # Create current context from time indexes per variable to include
        insample_y = create_context(insample_y=insample_y,
                                    insample_x_t=insample_x_t, outsample_x_t=outsample_x_t,
...
class XBasisTCN(nn.Module):
    def __init__(self, out_features, in_features, num_levels=4, kernel_size=2, dropout_prob=0):
        ...
        self.tcn = TemporalConvNet(num_inputs=in_features, num_channels=n_channels,
                                   kernel_size=kernel_size, dropout=dropout_prob)

    def transform(self, insample_x_t, outsample_x_t):
        input_size = insample_x_t.shape[2]

        # Filter the xbasis columns with the f_cols indexes
        x_t = t.cat([insample_x_t, outsample_x_t], dim=2)
...
class XBasisWavenet(nn.Module):
    def __init__(self, out_features, in_features, num_levels=4, kernel_size=3, dropout_prob=0):
        ...
        input_layer = [nn.Conv1d(in_channels=in_features, out_channels=out_features,
                                 kernel_size=kernel_size, padding=padding, dilation=2**0),
                       Chomp1d(padding), nn.ReLU(), nn.Dropout(dropout_prob)]
        conv_layers = []
        for i in range(1, num_levels):
            dilation = 2**i
            padding = (kernel_size - 1) * dilation
