...
# Cell
def filter_input_vars(x_t, t_cols, include_var_dict):
    ...
    for var in include_var_dict.keys():
        if len(include_var_dict[var])>0:
            ...
            if var != 'week_day':
                input_vars  += [x_t[:, t_col_idx-1, t_col_filter, :]] #-1 because y is not in x_t
            else:
                assert t_col_filter == [-1], f'Day of week must be of outsample not {t_col_filter}'
                day_var = x_t[:, t_col_idx-1, t_col_filter, [0]]  #-1 because y is not in x_t
...
class _ES(nn.Module):
    ...
    def forward(self, y, x, s_matrix, step_size, idxs):
        ...
        windows_y_insample = torch.zeros((n_windows, batch_size, input_size + n_t + n_s),
                                          device=self.device)
        ...
        for i, window in enumerate(windows_range):
            ...
            #TODO: HARDODED FOR EPF!!!!!!!!
            if n_t > 0:
                window_x_t = x[:, :, y_insample_start:(y_insample_end+output_size)]
                window_x_t = filter_input_vars(x_t=window_x_t, t_cols=self.t_cols,
...
class _RNN(nn.Module):
    def __init__(self, input_size, output_size, n_t, n_s, cell_type, dilations, state_hsize, add_nl_layer):
        ...
        for grp_num in range(len(self.dilations)):
            if grp_num == 0:
                input_size = self.input_size + self.n_t + self.n_s
            else:
