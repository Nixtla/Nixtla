...
__all__ = ['filter_input_vars', 'NBeatsBlock', 'NBeats', 'IdentityBasis', 'TrendBasis', 'SeasonalityBasis',
           'ExogenousBasisInterpretable', 'Chomp1d', 'ExogenousBasisWavenet', 'ExogenousBasisTCN']
...
def filter_input_vars(insample_y, insample_x_t, outsample_x_t, t_cols, include_var_dict):
    ...
    output_size = outsample_x_t.shape[2]
    input_size = insample_y.shape[1]
    assert input_size % output_size == 0, 'Filter_input_vars only works if input_size is multiple of output_size.'
    input_size_multiplier = int(input_size/output_size)
    ...
    batch_size, n_channels, _ = x_t.shape

    x_t = x_t.reshape(batch_size, n_channels, input_size_multiplier+1, output_size) # +1 because x_t has input+output

    input_vars = []
    slice_vars = []
    slice_var_flag = False
    for var in include_var_dict.keys():
        if len(include_var_dict[var])>0:
            t_col_idx    = t_cols.index(var) - 1 # Removing 'y' in position 0
            t_col_filter = include_var_dict[var]
            if var in slice_vars_names:
                assert t_col_filter == [-1], f'Day of week must be of outsample not {t_col_filter}'
                slice_var = x_t[:, t_col_idx, t_col_filter, [0]]
                slice_vars += [slice_var]
                slice_var_flag = True
            else:
    ...
    if slice_var_flag:
        slice_vars_filter = t.cat(slice_vars, dim=1)
        slice_vars_filter = slice_vars_filter.view(batch_size, -1)
        x_t_filter = t.cat([x_t_filter, slice_vars_filter], dim=1)
