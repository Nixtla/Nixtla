...
__all__ = ['accuracy_logits', 'init_weights', 'Nbeats']
...
from ...losses.pytorch import MAPELoss, MASELoss, SMAPELoss, MSELoss, MAELoss, RMSELoss, PinballLoss, QuadraticBarrierLoss
...
class Nbeats(object):
    ...
    def create_stack(self):
        ...
        if self.include_var_dict is not None:
            x_t_n_inputs = 0
            for key in self.include_var_dict.keys():
                if key in slice_vars:
                    x_t_n_inputs += 1
                else:
                    x_t_n_inputs += self.output_size * len(self.include_var_dict[key])
        else:
